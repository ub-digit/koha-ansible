...
my $preferences = {
    ...
    production => {
        OPACBaseURL => 'https://koha.ub.gu.se',
        patronselfregistrationlinkaddress => 'https://bibliotekskort.ub.gu.se/',
        staffClientBaseURL => 'https://koha-intra.ub.gu.se'
    }
};
...
my $plugin_preferences = {
    ...
    production => {
        'Koha::Plugin::Se::Gu::Ub::GetPrintData' => {
            api_url => 'https://bestall.ub.gu.se/api/print',
            api_key => $get_print_data_api_key,
        },
        'Koha::Plugin::Se::Ub::Gu::MarcImport' => {
            run_marc_command_command => $marc_command,
...
while (my ($pref, $value) = each %{$preferences->{$environment}}) {
    $dbh->do(
        q{UPDATE `systempreferences` SET `value` = ? WHERE `variable` = ?},
        undef, $value, $pref
    ) or die $dbh->errstr;
}
...
while (my ($plugin_class, $settings) = each %{$plugin_preferences->{$environment}}) {
    while (my ($plugin_key, $plugin_value) = each %{$settings}) {
        $dbh->do(
            q{ UPDATE `plugin_data` SET `plugin_value` = ? WHERE `plugin_class` = ? AND `plugin_key` = ?},
            undef, $plugin_value, $plugin_class, $plugin_key
        ) or die $dbh->errstr;
    }
}
